...
Plugin URI: https://monkeytime.be
...
Version: 0.7
Author URI: https://monkeytime.be
*/

/*  
	Copyright 2012  Laurent (KwarK) Bertrand  (email : contact@monkeytime.be)
	
	Small pizza donation @ https://monkeytime.be
...
function gcs_recolt_datas($content)
{
	if(is_single() || is_page())
	{
		...
		if(is_int($counter)) //To make sur if gcs_hit > (int) 1... (after a php warning, or others problems)
		{
			...
			$referer = preg_replace('#/$#', '', !empty(wp_get_referer()) ? wp_get_referer() : $_SERVER['HTTP_REFERER']);
			/*var_dump($from);*/
			/* echo '<pre>';
			print_r($referer);
			echo '</pre>'; */
